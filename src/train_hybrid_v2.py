"""
train_hybrid_v2.py — two-phase redesigned trainer for the Quantum Hybrid SBERT paper.

Key differences from v1 (train_hybrid.py):
...
from hybrid_heads import ClassicalMLPHead, PQCHead
...
def compute_metrics(logits: np.ndarray, labels: np.ndarray) -> dict:
    ...
    return {
        "f1": float(f1_score(labels.astype(int), preds, zero_division=0)),
        "acc": float(accuracy_score(labels.astype(int), preds)),
    }
...
def run_phase1(
    ...
    probe = nn.Linear(n_qubits, 1).to(DT)
    ...
    for ep in range(epochs):
        ...
        if m["f1"] > best_f1:
            ...
            if no_improve >= patience:
                print(f"  [phase1] early stop ep {ep}  best_val_f1={best_f1:.4f}")
                break

        if ep % 5 == 0 or ep == epochs - 1:
            print(
                f"  [phase1] ep {ep:>3}: loss={total_loss/max(nb,1):.4f}"
                f"  val_f1={m['f1']:.4f}  val_acc={m['acc']:.4f}"
            )
...
def run_phase2(
    ...
    for ep in range(epochs):
        ...
        if m_val["f1"] > best_f1:
            ...
            no_improve += 1
            if no_improve >= patience:
                print(f"  [phase2] early stop ep {ep}  best_val_f1={best_f1:.4f}")
                break

        if ep % 5 == 0 or ep == epochs - 1:
            print(
                f"  [phase2] ep {ep:>3}: loss={total_loss/max(nb,1):.4f}"
                f"  val_f1={m_val['f1']:.4f}  test_f1={m_te['f1']:.4f}"
...
def main() -> None:
    ...
    p.add_argument("--head", choices=["pqc", "mlp"], default="pqc")
    ...
    t_start = time.time()
    print(
        f"=== train_hybrid_v2: {args.reducer}+{args.head} | "
    ...
    ckpt_dir = ROOT / "results" / "v2" / "checkpoints"
    ...
    if args.head == "pqc":
        head = PQCHead(n_qubits=args.n_qubits, n_layers=args.n_layers).to(DT)
    else:
        head = ClassicalMLPHead(
            n_qubits=args.n_qubits, hidden_dim=args.mlp_hidden
        ).to(DT)
    print(f"   head: {head.__class__.__name__}  params={head.n_params}")
    ...
    out_path = out_dir / (
        f"hybrid_v2_{args.task}_{args.reducer}_{args.head}"
        ...
        json.dumps(
            {
                ...
                "phase1": {
                    "history": p1_history,
                    "best_val_f1": float(p1_best_val_f1),
                },
                "phase2": {
                    "history": p2_history,
                    "best_val_f1": float(p2_best_val_f1),
    ...
    print(f"[done] wrote results/v2/{out_path.name}")
