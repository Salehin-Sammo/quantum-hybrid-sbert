"""
train_hybrid_v4.py — v3 plus class-weighted BCE to avoid majority-class collapse.
    ...
    for ep in range(epochs):
        ...
        if ep % 5 == 0 or ep == epochs - 1:
            print(
                ...
                f"  val_macroF1={m_val['macro_f1']:.4f}  test_macroF1={m_te['macro_f1']:.4f}  test_f1={m_te['f1']:.4f}"
...
def main() -> None:
    ...
    t_start = time.time()
    print(
        f"=== train_hybrid_v4: {args.reducer}+{args.head} | "
    ...
    out_path = out_dir / (
        f"hybrid_v4_{args.task}_{args.reducer}_{args.head}"
    ...
    print(f"[done] wrote results/v4/{out_path.name}")
